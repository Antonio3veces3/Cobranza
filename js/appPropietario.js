...
function validacionExisteDeudores(nombres,apellidos,telefono){
    ...
    for(let i=0;i<existentes;i++){
        if(database.datos[indice].deudores[i].nombres==nombres && database.datos[indice].deudores[i].apellidos==apellidos || database.datos[indice].deudores[i].celular==telefono)
        return true;
    }
};
